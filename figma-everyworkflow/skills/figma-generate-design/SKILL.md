...
- Use this skill when the deliverable is a **Figma screen** (new or updated) composed of design system component instances, and you are translating **existing source code** into Figma.
- If the user wants to generate a Figma screen from **natural language, an HTML file, or a wireframe image** (i.e., there is no existing running code to translate), use [figma-generate-ui](../figma-generate-ui/SKILL.md) instead — it is the higher-level AI generation skill that calls this workflow internally.
...
> **Relationship to `figma-generate-ui`:** The `figma-generate-ui` skill is the user-facing entry point for AI-driven UI generation. It calls this skill's design system discovery and section-building workflow as its core engine. When loaded from `figma-generate-ui`, the steps below apply in full — the key difference is the input source (description/HTML/wireframe vs. source code).
...
| Skill | Role |
|---|---|
| [figma-generate-ui](../figma-generate-ui/SKILL.md) | Higher-level entry point: generates Figma screens from text, HTML, or wireframes using this skill's workflow |
| [figma-use](../figma-use/SKILL.md) | Plugin API rules — mandatory prerequisite before every `use_figma` call |
| [figma-implement-design](../figma-implement-design/SKILL.md) | Reverse direction: translates Figma designs into production code |
| [figma-generate-library](../figma-generate-library/SKILL.md) | Builds design system tokens and components from scratch |
