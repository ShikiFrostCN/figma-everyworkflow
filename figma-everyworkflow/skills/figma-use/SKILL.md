...
**If the user wants to generate a Figma screen from natural language, an HTML file, or a wireframe image**, load [figma-generate-ui](../figma-generate-ui/SKILL.md) — the AI-driven UI generation skill that uses `use_figma` + `search_design_system` + `figma-generate-design` to build complete screens from any input type.
...
## 10. Related Higher-Level Skills

These skills build on `figma-use` and provide structured workflows for common tasks. Load them when the task matches — they handle the orchestration so you don't have to write everything from scratch.

| Skill | When to use |
|---|---|
| [figma-generate-ui](../figma-generate-ui/SKILL.md) | Generate a complete Figma screen from natural language, HTML, or a wireframe image |
| [figma-generate-design](../figma-generate-design/SKILL.md) | Translate existing source code into a Figma screen using design system components |
| [figma-generate-library](../figma-generate-library/SKILL.md) | Build or update a design system (tokens, components, variants) from a codebase |
| [figma-implement-design](../figma-implement-design/SKILL.md) | Translate a Figma design into production-ready code |
| [figma-code-connect-components](../figma-code-connect-components/SKILL.md) | Map Figma components to code implementations via Code Connect |
